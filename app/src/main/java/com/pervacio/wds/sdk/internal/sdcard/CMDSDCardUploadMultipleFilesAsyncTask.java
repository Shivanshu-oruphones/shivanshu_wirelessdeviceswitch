...
public class CMDSDCardUploadMultipleFilesAsyncTask
        ...
        implements EMProgressHandler, CMDCopyFileProgressDelegate
{
    ...
    private final static String TAG = "UploadMultFilesAsync-SD";
    ...
    CMDSDCardUploadMultipleFilesAsyncTask(ArrayList<EMFileMetaData> aFilesToUpload,
                                          String aBackupPath) {
        mFilesToUpload     = aFilesToUpload;
        mCurrentFileIndex = 0;

        // mGoogleDriveAccess = aGoogleDriveAccess;
        mBackupPath = aBackupPath;
    ...
    public void runTask() {
        for(EMFileMetaData fileToUpload : mFilesToUpload)
        {
            ...
            String remoteFilePath = mBackupPath + "/" + subFolderBase + fileToUpload.mRelativePath + fileToUpload.mFileName;
            EMUtility.makePathForLocalFile(remoteFilePath);
            boolean copySuccessful = EMUtility.copyFile(new File(fileToUpload.mSourceFilePath), new File(remoteFilePath));

            int result = CMDError.CMD_RESULT_OK;
            if (!copySuccessful) {
                result = CMDError.CMD_SD_CARD_ERROR_COPYING_FILE_TO_SD_CARD;
            }
